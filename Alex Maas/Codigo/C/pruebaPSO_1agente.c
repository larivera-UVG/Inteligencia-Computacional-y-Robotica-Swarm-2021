...
#define IP_LENGTH 15
#define PUERTO 2001				// puerto seleccionado
...
static double posicion_robot_X;
static double posicion_robot_Y;
static double rad;
...
// Variable de recibir
char buffer_recibir[MSG_SIZE];  		// almacenar mensajes
char *token;
...
void *receiving(void *ptr)
{
    int *sock, n, ret;
    int i = 0;
    sock = (int *)ptr;                                			// identificador de socket
    unsigned int length = sizeof(struct sockaddr_in); 			// tamaño de la estructura
    struct sockaddr_in from;

    while (1)
    {
        memset(buffer_recibir, 0, MSG_SIZE); 				// se limpia el buffer
        // recibir mensaje
        ...
        i = 0;
        ret=strncmp(buffer_recibir, "T1" ,2);				// identificador si son coordenadas o informacion de agentes

        if(ret != 0){	
	// si es informacion de agentes hacer esto 
...
int main(int argc, char *argv[])
{
    ...
    int sock, n;
    unsigned int length = sizeof(struct sockaddr_in);           // tamaño de la estructura
    char buffer_enviar1[MSG_SIZE], buffer_enviar2[MSG_SIZE];    // almacenar mensajes.
    ...
    FILE *file;
    strcpy(IP_broadcast, "10.0.0.255"); 			// Revisar ifconfig (se debe modificar de acuerto a la red)
    ...
    // Se permite comunicacion del tipo broadcast
    ...
    pthread_create(&hilo_envio, NULL, receiving, (void *)&sock);	 	// hilo de programacion para recepcion 
