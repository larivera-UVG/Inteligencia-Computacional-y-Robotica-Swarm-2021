...
def send_file(sck: socket.socket, filename):
    ...
    sck.sendto(struct.pack("<Q", filesize),server_address)
    # Enviar el archivo en bloques de 1024 bytes.
    pic = open(filename, 'rb')
    chunk = pic.read(1024)
    while chunk:
        print("Sending Picture")
        sck.sendto(chunk,server_address)
        chunk = pic.read(1024)
    pic.close()
    print("Done sending")
...
while True:
    ...
    try:
        while True:
            #sock.sendto(mensaje,server_address)
            ...
            if data == b'6':
                ...
                sock.sendto(distance.encode("utf-8"),server_address)
                time.sleep(1)
                sock.sendto(distance1.encode("utf-8"),server_address)
                time.sleep(1)
