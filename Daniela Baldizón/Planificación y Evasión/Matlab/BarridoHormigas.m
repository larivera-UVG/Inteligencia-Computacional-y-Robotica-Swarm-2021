...
sweep = 5:5:100;
repetitions = 15;
Hormigas_sweep_data = cell(numel(sweep) * repetitions + 1, 5);
Hormigas_sweep_data(1, :) = {'tiempo', 'costo', 'iteraciones', 'path', 'Hormigas'};
sweep_count = 1;
t_acumulado = 0;
...
%             caminos(k) = numel(ants(k).path);
    ...
    if (t > t_max)
        ...
        Hormigas_sweep_data(sweep_count + 1, :) = {tiempofinal, 0, t - 1, best_path, hormigas};
    else
        moda =  mode(L(:, t-1));
        ...
        tiempofinal = toc(timer);
        Hormigas_sweep_data(sweep_count + 1, :) = {tiempofinal, L(len_prob, t-1), t - 1, best_path, hormigas};

    end
    t_acumulado = t_acumulado + tiempofinal;
    if t_acumulado >= 60
        disp("Guardando...")
        save('sweep_data', 'Hormigas_sweep_data', '-append')
        t_acumulado = 0;
    end
